...
def calcBestWayToPayacc1( paymentbox, amountbox, inflt1, inflt2, drawingPanel, answer ) :
	...
	for i in range(0, len(accounts)):
		...
		if (tempProfit == -1):
			print "Þú átt nú þegar ".decode("utf-8") + str(amount) + " kr.".decode("utf-8")
			s += "Þú átt nú þegar ".decode("utf-8") + str(amount) + " kr.".decode("utf-8")
			return
...
def makeAccount(name, balance, interests, reqtime, indexadj, answer, accountlist):
	...
	if( acc_interests is False or acc_reqtime is False or acc_balance is False ):
		...
		answer.SetLabel(s)
		return
	accType = AccountType(acc_name, acc_reqtime, float(acc_interests)/100.0, acc_indexadj, -1, -1)
	...
	accountlist.SetStringItem(index, 2, str(account.acctype.interests*100))

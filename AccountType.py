...
def calcAccProfit(account):
    profit = (account.getInterests()/100.0)*account.getBalance()-account.getBalance()
    return profit
...
def readAccountTypes():
    ...
    for item in listi:
        temp = item.split(",")
        acc = AccountType(temp[0],int(temp[1]),int(temp[2]),float(temp[3]),float(temp[4]),int(temp[5]),int(temp[6]),int(temp[7]))
